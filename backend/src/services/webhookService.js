...
class WebhookService {
  constructor() {
    ...
    this.ghlApiKey = process.env.GHL_API_KEY || null;
    this.ghlApiBaseUrl = 'https://rest.gohighlevel.com/v1';
  ...
  /**
   * Cerca un contatto in GHL per email
   * @param {string} email - Email del contatto
   * @returns {Promise<object|null>} - Contatto trovato o null
   */
  async ghlLookupContact(email) {
    if (!this.ghlApiKey) {
      console.log('[GHL_API] API key not configured');
      return null;
    }

    try {
      const response = await fetch(
        `${this.ghlApiBaseUrl}/contacts/lookup?email=${encodeURIComponent(email)}`,
        {
          method: 'GET',
          headers: {
      ...
      if (contact) {
        console.log(`[GHL_API] Contact found for ${email}: ${contact.id}`);
      } else {
        console.log(`[GHL_API] No contact found for ${email}`);
      }

      return contact;
    } catch (error) {
      console.error(`[GHL_API] Lookup failed for ${email}:`, error.message);
      return null;
    }
  }

  /**
   * Aggiunge uno o più tag a un contatto GHL
   * @param {string} contactId - ID del contatto GHL
   * @param {string|string[]} tags - Tag da aggiungere
   * @returns {Promise<boolean>} - true se successo
   */
  async ghlAddTags(contactId, tags) {
    if (!this.ghlApiKey) {
      console.log('[GHL_API] API key not configured');
      return false;
    }
    ...
    try {
      const response = await fetch(
        `${this.ghlApiBaseUrl}/contacts/${contactId}/tags`,
        {
          method: 'POST',
          headers: {
      ...
      console.log(`[GHL_API] Tags added to contact ${contactId}:`, tagArray);
      return true;
    } catch (error) {
      console.error(`[GHL_API] Add tags failed for contact ${contactId}:`, error.message);
      return false;
    }
  }

  /**
   * Rimuove uno o più tag da un contatto GHL
   * @param {string} contactId - ID del contatto GHL
   * @param {string|string[]} tags - Tag da rimuovere
   * @returns {Promise<boolean>} - true se successo
   */
  async ghlRemoveTags(contactId, tags) {
    if (!this.ghlApiKey) {
      console.log('[GHL_API] API key not configured');
      return false;
    }
    ...
    try {
      ...
      console.log(`[GHL_API] Tags removed from contact ${contactId}:`, tagArray);
      return true;
    } catch (error) {
      console.error(`[GHL_API] Remove tags failed for contact ${contactId}:`, error.message);
      return false;
    }
  }

  /**
   * Aggiunge tag a un contatto cercandolo per email
   * Metodo helper che combina lookup + addTags
   * @param {string} email - Email del contatto
   * @param {string|string[]} tags - Tag da aggiungere
   * @returns {Promise<boolean>} - true se successo
   */
  async addTagByEmail(email, tags) {
    const contact = await this.ghlLookupContact(email);
    if (!contact) {
      console.log(`[GHL_API] Cannot add tags - contact not found: ${email}`);
      return false;
    }
    return this.ghlAddTags(contact.id, tags);
  }

  /**
   * Rimuove tag da un contatto cercandolo per email
   * Metodo helper che combina lookup + removeTags
   * @param {string} email - Email del contatto
   * @param {string|string[]} tags - Tag da rimuovere
   * @returns {Promise<boolean>} - true se successo
   */
  async removeTagByEmail(email, tags) {
    const contact = await this.ghlLookupContact(email);
    if (!contact) {
      console.log(`[GHL_API] Cannot remove tags - contact not found: ${email}`);
      return false;
    }
    return this.ghlRemoveTags(contact.id, tags);
  }

  /**
   * Notifica GHL che un utente migrato ha confermato la migrazione
   * Aggiunge il tag "migrazione-confermata" per uscire dall'automazione reminder
   * @param {string} email - Email dell'utente
   * @param {string} migratedFrom - Servizio di provenienza (per tag specifico)
   * @returns {Promise<boolean>} - true se successo
   */
  async notifyMigrationConfirmed(email, migratedFrom = null) {
    const tags = ['migrazione-confermata'];

    // Aggiungi tag specifico per servizio se disponibile
    if (migratedFrom) {
      tags.push(`migrazione-${migratedFrom}`);
    }
